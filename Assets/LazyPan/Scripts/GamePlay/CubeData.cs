...
namespace LazyPan {
    public class CubeData : MonoBehaviour {
        ...
        public bool GetData<T>(string sign, out T t) {
            if (typeof(T) == typeof(FloatData)) {
            ...
            } else if (typeof(T) == typeof(BoolData)) {
        ...
        public bool Get<T>(string sign, out T t) {
            if (typeof(T) == typeof(float)) {
                foreach (FloatData data in Floats) {
                    if (data.Sign == sign) {
                        t = (T)Convert.ChangeType(data.Float, typeof(T));
                        return true;
                    }
                }
                ...
                foreach (StringData data in Strings) {
                    if (data.Sign == sign) {
                        t = (T)Convert.ChangeType(data.String, typeof(T));
                        return true;
                    }
                }
            }
